  ...
  return (
    <div className="h-screen w-full overflow-hidden relative" style={{ background: 'var(--parchment)' }}>
      ...
      <div className="absolute top-4 left-4 right-4 z-1000 flex items-center gap-3">
        {/* Logo */}
        <div
          className="glass rounded-2xl px-4 py-2.5 flex items-center gap-2.5 shrink-0 border border-white/60"
        ...
        <form onSubmit={handleSearch} className="flex-1 max-w-xl">
          <div className="relative">
            <input
              ...
              placeholder="Where shall we explore? (Paris, Tokyo, Rome...)"
              className="w-full glass border border-white/60 rounded-2xl px-5 py-3 pr-12 text-[15px] focus:outline-none focus:ring-2 transition-shadow"
              style={{
                color: 'var(--ink)',
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 p-2.5 rounded-xl transition-all duration-200 disabled:opacity-40"
        ...
        <button
          ...
          className="glass border border-white/60 rounded-2xl p-3 transition-all shrink-0 hover:scale-105 active:scale-95"
          style={{
        ...
        {(hasDiscoveredPois || itinerary) ? (
          <button
            ...
            className="glass border border-white/60 rounded-2xl px-4 py-2.5 text-sm font-medium transition-all shrink-0 hover:scale-105 active:scale-95"
      ...
      {showTripSettings && !hasDiscoveredPois ? (
        <div className="absolute top-20 left-4 z-999 animate-slide-down">
          <div
            className="glass rounded-2xl border border-white/60 p-5 w-80"
          ...
          >
            ...
            <div className="space-y-5">
              <div>
                <label
                  className="text-xs font-medium uppercase tracking-widest mb-2.5 block"
      ...
      {/* ─── Streaming Progress ─── */}
      {isStreaming ? (
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-1000 animate-slide-up">
          <div
            className="glass-dark text-white px-7 py-3.5 rounded-full flex items-center gap-4"
            style={{ boxShadow: 'var(--shadow-float)' }}
          >
            ...
            <span className="text-sm font-medium tracking-wide" style={{ fontFamily: 'var(--font-body)' }}>
      ...
      {error ? (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 z-1000 animate-slide-down">
          ...
          >
            ...
            <button onClick={clearError} className="ml-2 opacity-60 hover:opacity-100 transition-opacity">
              {closeIcon14}
            </button>
          </div>
        </div>
      ...
      {selectedPoi && !itinerary ? (
        <div className="absolute bottom-16 left-4 z-1001 animate-slide-up">
          <POIPreviewCard
            poi={selectedPoi}
      ...
      {/* ─── POI Card — Itinerary ─── */}
      {selectedPoi && itinerary ? (
        <div className="absolute bottom-6 left-4 z-1001 animate-slide-up">
          <POIPreviewCard
            poi={selectedPoi}
      ...
      {hasDiscoveredPois && !itinerary ? (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-1000 animate-slide-up">
          <button
            ...
            className="group relative overflow-hidden px-8 py-4 rounded-2xl font-semibold text-[17px] tracking-tight transition-all duration-300 ease-out flex items-center gap-3"
            style={{
              fontFamily: 'var(--font-body)',
              ...(acceptedCount > 0
                ? {
